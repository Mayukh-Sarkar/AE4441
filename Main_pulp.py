# -*- coding: utf-8 -*-
"""
Created on Mon Feb  1 14:02:58 2021
...
solver = plp.CPLEX_CMD(path = r'C:\Program Files\IBM\ILOG\CPLEX_Studio_Community201\cplex\bin\x64_win64\cplex.exe')
...
class DroneScanningOpt:
    def __init__(self, area_shape, amount_of_drones_used, location_charging_points, minimum_charge,
        ...
        self.pm = plp.LpProblem('Modelname')  # Name of the gurobi model
    ...
    def dec_var(self):
        ...
        for k in range(self.drone_amount):
            ...
            for i in range(len(self.area_coord)):
                ...
                for j in range(self.i):
                    if self.link(i, j) < 20 and i != j and self.i - len(self.location_charg_point) > j \
                            and self.i - len(self.location_charg_point) > i:

                        self.x_vars[i, j, k] = plp.LpVariable(cat=plp.LpInteger, upBound=2, lowBound=0,
        ...
        for k in range(self.drone_amount):
            for i in range(len(self.area_coord)):
                self.q[i, k] = plp.LpVariable(cat=plp.LpContinuous,upBound=100, 
                                              name='Charge at node ' + str(i) + ' Drone ' + str(k))
                self.obj_charge += self.q[i, k]
        # Time Variables
        for k in range(self.drone_amount):
            for i in range(len(self.area_coord)):
                self.time[i, k] = plp.LpVariable(cat=plp.LpContinuous,
    ...
    def create_obj_funct(self):
        ...
        self.pm.sense = plp.LpMinimize
        self.pm.setObjective(self.obj_function)
    ...
    def general_constr(self):
        ...
        for k in range(self.drone_amount):
            for h in range(0, self.i):
                self.pm.addConstraint(plp.LpConstraint(plp.lpSum(self.x_vars[i, h, k] for i in self.value_list[h])
                                  - plp.lpSum(self.x_vars[h, j, k] for j in self.value_list[h]) == 0))

        # Making sure all the nodes are visited
        for i in range(0, self.i):
            if i not in self.charge_nodes:
                self.pm.addConstraint(plp.LpConstraint(plp.lpSum(self.x_vars[i, j, k] for j in self.value_list[i]
                                              for k in range(self.drone_amount)) >= 1))

        # Making sure all the drones leave the home station
        for k in range(self.drone_amount):
            self.pm.addConstraint(plp.LpConstraint(plp.lpSum(self.x_vars[0, j, k] for j in self.value_list[0]) == 1))
    ...
    def charge_constr(self):
        ...
        for k in range(self.drone_amount):
            for i in range(0, self.i):
                self.pm.addConstraint(plp.LpConstraint(self.q[i, k] >= self.min_charge))
            ...
            for i in range(0, self.i):
                for j in self.value_list[i]:
                    if j not in self.charge_nodes:
                        self.pm.addConstraint(plp.LpConstraint(self.q[i, k] - self.q[j, k] - self.charge_constant * self.link(i, j)
                                          * self.x_vars[i, j, k] - self.charge_scan + (
                                                      1 - self.x_vars[i, j, k]) * self.M >= 0))
                    if j in self.charge_nodes:
                        self.pm.addConstraint(plp.LpConstraint(self.q[i, k] - self.charge_constant * self.link(i, j) * self.x_vars[i, j, k] +
                                          (1 - self.x_vars[i, j, k]) * self.M >= 0))
    ...
    def time_constr(self):
        ...
        for k in range(self.drone_amount):
            for i in range(0, self.i):
                for j in self.value_list[i]:
                    if j not in self.charge_nodes:
                        self.pm.addConstraint(plp.LpConstraint(self.time[i, k] - self.time[j, k] + self.time_constant * self.link(i, j)
                ...
                for i, txt in enumerate(n):
                    if txt != 100:
                        ...
                        pyplot.annotate(text_on_node, (np.array(self.area_coord)[i, 1], np.array(self.area_coord)[i, 2])
                                        , color='white')
        pyplot.title("Objective Value: " + str(round(self.plp.objVal, 1)) + " || " + "Run time: " +
                     str(round(self.plp.RunTime, 2)) + 's || ' + "Amount of Drones: " + str(self.drone_amount))
    ...
    def run_optimization(self):
        ...
        self.pm.writeLP('pulp.lp')  # write to LP file
        ...
        self.pm.solve(solver)
        #try:
        #self.pm.printAttr('X')#print(x.varValue)
        #self.result = self.plp.getAttr('X')
        print(self.pm.variables()[0].name)
        print(self.pm.variables()[40].value())
        print("Objective Function Value = ", value(self.pm.objective))
    ...
    for i in range(x_range_charg):
        for j in range(y_range_charg):
            Model_loop = DroneScanningOpt(area_shape='map_locations', amount_of_drones_used=1,
                                          ...
                                          scanning_consumption=2, name="map_location_run1", logging_obj=wb,
                                          plotting=False)
            Model_loop.run_optimization()
            threedgraph[i][j] = (Model_loop.get_objective_value())
...
#         cost_2 = []
#         cost_3 = []
#         minimum_charge_1 = list(range(0,11))
#         scanning_consumption_1 = list(range(0,11))
...
#            #                    scanning_consumption=2, name= str(f+10), logging_obj=wb, plotting=True,
#            #                    area_length=3, area_width=2)
#            # Model_2 = DroneScanningOpt(area_shape='map_locations', amount_of_drones_used=2,
#            #                    location_charging_points=[[5, 10]], minimum_charge=f, scanning_time=15,
#            #                    scanning_consumption=2, name=str(f+21), logging_obj=wb, plotting=True,
#            #                    area_length=3, area_width=2)
#            # Model_3 = DroneScanningOpt(area_shape='map_locations', amount_of_drones_used=3,
#            #                   location_charging_points=[[5, 10]], minimum_charge= f, scanning_time=15,
#            #                    scanning_consumption=2, name= str(f+32), logging_obj=wb, plotting=True,
#            #                    area_length=3, area_width=2)
#            # Model_1.run_optimization()
#            #Model_2.run_optimization()
#            # Model_3.run_optimization()
            
#            # cost_1.append(Model_1.get_objective_value())
#            # cost_2.append(Model_2.get_objective_value())
#            # cost_3.append(Model_3.get_objective_value())
        
#       #  pyplot.figure()
#       #  pyplot.plot(minimum_charge_1,cost_1,'r-p',label = '1 drone')
#      #
#       #  pyplot.plot(minimum_charge_1,cost_2,'g-v' , label = '2 drones')
        
#       #  pyplot.plot(minimum_charge_1,cost_3,'b-*' , label = '3 drones')
#       #  pyplot.xlabel('Minimum charge level required in the drone(%)')
#        # pyplot.ylabel('Cost variation( $)')
#        # pyplot.legend()
#        # pyplot.title('Cost variation wrt minimum charge requirement')
        
#       #  pyplot.show()
#       #  pyplot.savefig('min_charge.jpeg')
#         for k in scanning_consumption_1:
#             Model_1 = DroneScanningOpt(area_shape='map_locations', amount_of_drones_used=1,
#                                location_charging_points=[[5, 10]], minimum_charge= 5, scanning_time=15,
#                              scanning_consumption= k, name= str(k + 50), logging_obj=wb, plotting=True,
#                               area_length=3, area_width=2)
#             Model_2 = DroneScanningOpt(area_shape='map_locations', amount_of_drones_used=2,
#                                location_charging_points=[[5, 10]], minimum_charge=5, scanning_time=15,
#                                scanning_consumption=k, name= str(k + 61), logging_obj=wb, plotting=True,
#                                area_length=3, area_width=2)
#             Model_3 = DroneScanningOpt(area_shape='map_locations', amount_of_drones_used=3,
#                                location_charging_points=[[5, 10]], minimum_charge= 5, scanning_time=15,
#                                scanning_consumption=k, name= str(k +  80), logging_obj=wb, plotting=True,
#                            area_length=3, area_width=2)
#             Model_1.run_optimization()
#             #Model_2.run_optimization()
#            # Model_3.run_optimization()
...
#         pyplot.legend()
#         pyplot.show()
#         pyplot.savefig('scan_charge.jpeg')

Model_1 = DroneScanningOpt(area_shape='rectangle', amount_of_drones_used=1,
                                location_charging_points=[[5, 10]], minimum_charge= 5, scanning_time=15,
                              scanning_consumption= 0.01, name= 'hallo', logging_obj=wb, plotting=True,
                               area_length=3, area_width=2)
