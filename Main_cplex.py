...
class DroneScanningOpt:
    ...
    def dec_var(self):
            ...
            for i in range(len(self.area_coord)):
                ...
                for j in range(self.i):
                    if self.link(i, j) < 20 and i != j and self.i - len(self.location_charg_point) > j \
                        ...
                        self.x_vars[i, j, k] =  self.cm.integer_var( ub=2,  lb=0,
                                                              name='Link' + str(i) + '-' + str(
                                                                  j) + '_' + 'Drone ' + str(k))
                        lst.append(j)
        ...
        for k in range(self.drone_amount):
            for i in range(len(self.area_coord)):
                self.q[i, k] =  self.cm.continuous_var(  ub=100, 
                                              name='Charge at node ' + str(i) + ' Drone ' + str(k))
                self.obj_charge += self.q[i, k]
        # Time Variables
        for k in range(self.drone_amount):
            for i in range(len(self.area_coord)):
                self.time[i, k] =  self.cm.continuous_var( 
    ...
    def create_obj_funct(self):
        ...
        self.cm.minimize(self.obj_function)
    ...
    def general_constr(self):
        ...
        for k in range(self.drone_amount):
            for h in range(0, self.i):
                self.cm.add_constraint(self.cm.sum(self.x_vars[i, h, k] for i in self.value_list[h])
                                  - self.cm.sum(self.x_vars[h, j, k] for j in self.value_list[h]) == 0)

        # Making sure all the nodes are visited
        for i in range(0, self.i):
            if i not in self.charge_nodes:
                self.cm.add_constraint(self.cm.sum(self.x_vars[i, j, k] for j in self.value_list[i]
                                              for k in range(self.drone_amount)) >= 1)

        # Making sure all the drones leave the home station
        for k in range(self.drone_amount):
            self.cm.add_constraint(self.cm.sum(self.x_vars[0, j, k] for j in self.value_list[0]) == 1)
    ...
    def charge_constr(self):
        ...
        for k in range(self.drone_amount):
            for i in self.charge_nodes:
                ...
                self.cm.add_constraint(self.q[i, k] == 100)

        # Never go below a certain battery level
        for k in range(self.drone_amount):
            for i in range(0, self.i):
                self.cm.add_constraint(self.q[i, k] >= self.min_charge)
            ...
            for i in range(0, self.i):
                for j in self.value_list[i]:
                    if j not in self.charge_nodes:
                        self.cm.add_constraint(self.q[i, k] - self.q[j, k] - self.charge_constant * self.link(i, j)
                                          * self.x_vars[i, j, k] - self.charge_scan + (
                                                      1 - self.x_vars[i, j, k]) * self.M >= 0)
                    if j in self.charge_nodes:
                        self.cm.add_constraint( self.q[i, k] - self.charge_constant * self.link(i, j) * self.x_vars[i, j, k] +
                                          (1 - self.x_vars[i, j, k]) * self.M >= 0)
    ...
    def time_constr(self):
        ...
        for k in range(self.drone_amount):
            self.cm.add_constraint(self.time[0, k] == 0)
        ...
        for k in range(self.drone_amount):
            for i in range(0, self.i):
                for j in self.value_list[i]:
                    if j not in self.charge_nodes:
                        self.cm.add_constraint(self.time[i, k] - self.time[j, k] + self.time_constant * self.link(i, j)
...
# Model_1 = DroneScanningOpt(area_shape='map_locations', amount_of_drones_used=1,
#                                  location_charging_points=[[5, 10]], minimum_charge= 5, scanning_time=15,
#                                scanning_consumption= 0.01, name= str(50), logging_obj=wb, plotting=True,
#                                 area_length=3, area_width=2)
